...
class LanguageToolSupport
{
	...
	private HighlightPainter redPainter; // a red color highlight painter for
											// marking spelling errors
	private HighlightPainter bluePainter; // a blue color highlight painter for
											// marking grammar errors
	...
	public final UndoRedoSupport undo;
			...
			UndoRedoSupport support)
	{
		...
		this.undo = new UndoRedoSupport(textComponent, this.messages);
		init();
	}
	...
	private void fireEvent(LanguageToolEvent.Type type, Object caller)
	{
		...
		for (int i = listeners.length - 2; i >= 0; i -= 2)
		{
			if (listeners[i] == LanguageToolListener.class)
			{
				...
				((LanguageToolListener) listeners[i + 1])
						.languageToolEventOccurred(event);
			}
		}
	}
	...
	void reloadConfig()
	{
		...
		{
			// FIXME
			// we could skip a full check if the user disabled but didn't enable
			// rules
	...
	private void reloadLanguageTool(Language language)
	{
		try
		{
			...
			languageTool = new MultiThreadedJLanguageTool(language,
	...
	private void init()
	{
		...
		/**
		 * Warm-up: we have a lot of lazy init in LT, which causes the first
		 * check to be very slow (several seconds) for languages with a lot of
		 * data and a lot of rules. We just assume that the default language is
		 * the language that the user often uses and init the LT object for that
		 * now, not just when it's first used. This makes the first check feel
		 * much faster:
		 */
		...
		this.textComponent.getDocument().addDocumentListener(
				new DocumentListener()
				{
					...
					{
						if (e.getDocument().getLength() == e.getLength()
								&& config.getAutoDetect())
						{
	...
	private void showPopup(MouseEvent event)
	{
		if (documentSpans.isEmpty()
				&& languageTool.getDisabledRules().isEmpty())
		{
		...
		if (span != null)
		{
			JLabel msgItem = new JLabel("<html>"
					+ span.msg.replace("<suggestion>", "<b>").replace(
							"</suggestion>", "</b>") + "</html>");
			msgItem.setToolTipText(span.desc.replace("<suggestion>", "")
					.replace("</suggestion>", ""));
			...
			JMenuItem ignoreItem = new JMenuItem(
					messages.getString("guiTurnOffRule"));
		...
		{
			JMenu activateRuleMenu = new JMenu(
					messages.getString("guiActivateRule"));
		...
		for (String category : categories.keySet())
		{
					...
					&& (categories.keySet().size() - count > 1))
			{
				JMenu more = new JMenu(
						messages.getString("guiActivateRuleMoreCategories"));
				parent.add(more);
				parent = more;
			}
		}
	...
	private void createRulesMenu(JMenu parent, List<Rule> rules)
	{
		...
		for (Rule rule : rules)
		{
			...
			{
				JMenu more = new JMenu(
						messages.getString("guiActivateRuleMoreRules"));
				menu.add(more);
				menu = more;
			}
		}
	...
	private void applySuggestion(String str, int start, int end)
	{
		if (end < start)
		{
			throw new IllegalArgumentException("end before start: " + end
					+ " < " + start);
		}
	...
	public void checkDelayed(Object caller)
	{
		...
		checkExecutor.schedule(new RunnableImpl(caller), millisecondDelay,
	...
	{
		check.getAndIncrement();
		checkExecutor.schedule(new RunnableImpl(caller), 0,
				TimeUnit.MILLISECONDS);
	}
	...
	Language autoDetectLanguage(String text)
	{
		...
		try
		{
			lang = Language.getLanguageForShortName(langIdentifier
					.getLanguage());
		...
		if (lang.hasVariant())
		{
			// UI only shows variants like "English (American)", not just
			// "English", so use that:
	...
	private synchronized List<RuleMatch> checkText(final Object caller)
			throws IOException
	{
		if (this.mustDetectLanguage)
		{
			...
			{
				Language detectedLanguage = autoDetectLanguage(this.textComponent
						.getText());
				if (!detectedLanguage.equals(this.languageTool.getLanguage()))
				{
					...
					{
						fireEvent(LanguageToolEvent.Type.LANGUAGE_CHANGED,
								caller);
					}
					else
					{
						try
						{
							SwingUtilities.invokeAndWait(new Runnable()
							{
								...
								{
									fireEvent(
											LanguageToolEvent.Type.LANGUAGE_CHANGED,
											caller);
								}
							});
		...
		else
		{
			try
			{
				SwingUtilities.invokeAndWait(new Runnable()
				{
					...
					public void run()
					{
						fireEvent(LanguageToolEvent.Type.CHECKING_STARTED,
								caller);
					}
				});
			} catch (InterruptedException ex)
		...
		final List<RuleMatch> matches = this.languageTool
				.check(this.textComponent.getText());
		int v = check.get();
		if (v == 0)
		{
			if (!SwingUtilities.isEventDispatchThread())
			{
				SwingUtilities.invokeLater(new Runnable()
				{
					...
					public void run()
					{
						...
						fireEvent(LanguageToolEvent.Type.CHECKING_FINISHED,
								caller);
					}
				});
			}
	...
	{
		for (Highlighter.Highlight hl : textComponent.getHighlighter()
				.getHighlights())
		{
	...
	private void prepareUpdateHighlights(List<RuleMatch> matches,
			List<Span> spans)
	{
		ruleMatches.clear();
		documentSpans.clear();
	...
	private void updateHighlights()
	{
		...
		for (Span span : documentSpans)
		{
			...
			if (ITSIssueType.Misspelling.equals(span.rule
	...
	private void showDialog(Component parent, String title, String message,
			Rule rule)
	{
		Tools.showRuleInfoDialog(parent, title, message, rule, messages,
				languageTool.getLanguage().getShortNameWithCountryAndVariant());
	}
	
	private static class HighlightPainter extends
			DefaultHighlighter.DefaultHighlightPainter
	{
		
		private static final BasicStroke OO_STROKE1 = new BasicStroke(1.0f,
				BasicStroke.CAP_ROUND, BasicStroke.JOIN_ROUND, 10.0f,
				new float[]
				{ 3.0f, 5.0f }, 2);
		private static final BasicStroke OO_STROKE2 = new BasicStroke(1.0f,
				BasicStroke.CAP_ROUND, BasicStroke.JOIN_ROUND, 10.0f,
				new float[]
				{ 1.0f, 3.0f }, 3);
		private static final BasicStroke OO_STROKE3 = new BasicStroke(1.0f,
				BasicStroke.CAP_ROUND, BasicStroke.JOIN_ROUND, 10.0f,
				new float[]
				{ 3.0f, 5.0f }, 6);
		private static final BasicStroke ZIGZAG_STROKE1 = new BasicStroke(1.0f,
				BasicStroke.CAP_ROUND, BasicStroke.JOIN_ROUND, 10.0f,
				new float[]
				{ 1.0f, 1.0f }, 0);
